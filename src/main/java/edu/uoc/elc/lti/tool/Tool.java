...
public class Tool {
  ...
  public AccessTokenResponse getAccessToken() throws IOException, BadToolProviderConfigurationException {
    ...
    if (accessTokenResponse == null) {
      AccessTokenRequestHandler accessTokenRequestHandler = new AccessTokenRequestHandler(kid, registration, toolBuilders.getClientCredentialsTokenBuilder(),
  ...
  private void createUser(String subject) {
    this.user = User.builder().id(subject).givenName(this.claimAccessor.get(ClaimsEnum.GIVEN_NAME)).familyName(this.claimAccessor.get(ClaimsEnum.FAMILY_NAME))
        .middleName(this.claimAccessor.get(ClaimsEnum.MIDDLE_NAME)).picture(this.claimAccessor.get(ClaimsEnum.PICTURE))
        .email(this.claimAccessor.get(ClaimsEnum.EMAIL)).name(this.claimAccessor.get(ClaimsEnum.NAME)).build();
  }
  ...
  public DeepLinkingClient getDeepLinkingClient() {
    ...
    return new DeepLinkingClient(toolBuilders.getDeepLinkingTokenBuilder(), getIssuer(), registration.getClientId(), kid, this.claimAccessor.getAzp(),
        getDeploymentId(), this.claimAccessor.get(ClaimsEnum.NONCE), getDeepLinkingSettings());
  }
  ...
  public String getOidcAuthUrl(LoginRequest loginRequest) throws URISyntaxException {
    final LoginResponse loginResponse = LoginResponse.builder()
        ...
        .redirect_uri(loginRequest.getTarget_link_uri()).login_hint(loginRequest.getLogin_hint()).state(new BigInteger(50, new SecureRandom()).toString(16))
        .nonce(new BigInteger(50, new SecureRandom()).toString(16)).lti_message_hint(loginRequest.getLti_message_hint()).build();
  ...
  public Tool(final Registration registration, final ClaimAccessor claimAccessor, final OIDCLaunchSession oidcLaunchSession, final ToolBuilders toolBuilders,
      final String kid) {
